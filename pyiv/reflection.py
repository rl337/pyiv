    ...
    def discover_implementations(
        ...
        if reg["recursive"]:
            self._scan_submodules_recursive(
                package, package_path, interface, reg, implementations
            )
    ...
    def _scan_submodules_recursive(
        ...
        for submodule_name, submodule in submodules.items():
            ...
            for name, obj in inspect.getmembers(submodule):
                if self._is_implementation(obj, interface, reg["pattern"]):
                    # Use full path for submodule classes
                    full_name = f"{submodule_name}.{name}"
                    implementations[full_name] = obj
            
            # Recursively scan nested submodules
            nested_path = f"{package_path}.{submodule_name}"
            self._scan_submodules_recursive(
                submodule, nested_path, interface, reg, implementations
            )
