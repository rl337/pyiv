...
class CommandRunner:
    ...
    def discover_commands(
        self, package_path: str, pattern: Optional[str] = None, recursive: bool = True
            ...
            try:
                ...
                if isinstance(self.config, ReflectionConfig):
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Reflection discovery failed: {e}, falling back to manual discovery"
                        )
            except Exception as e:
        ...
        try:
            ...
            if recursive:
                for importer, modname, ispkg in pkgutil.iter_modules(
                    getattr(package, "__path__", []), package_path + "."
                ):
                    if ispkg:
                        continue
                        ...
                        for name, obj in inspect.getmembers(module, inspect.isclass):
                            if (
                                ...
                                try:
                                    ...
                                    logger.debug(
                                        f"Failed to get name from command {obj.__name__}: {e}"
                                    )
                                    continue
                    except Exception as e:
        ...
        for cmd_name, cmd_class in commands.items():
            try:
                ...
                cmd_parser = subparsers.add_parser(
                    cmd_name, help=description, aliases=cmd_class.get_aliases()
                )
                ...
                if subcommands:
                    ...
                    self._register_commands(
                        sub_subparsers, subcommand_dict, parent_path + [cmd_name]
                    )
            except Exception as e:
